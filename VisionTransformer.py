...
class ImagePreprocesser:
    """
    --input
    n_patches: number of patches (if we want x^2 cube patches from image, n_patch=x)
    """
...
class Block(nn.Module):
    def __init__(self, head_size, d_emb, num_heads, dropout):
        """
        --input
        num_heads: number of heads,
        head_size: maximum context length,
...
class VisionTransformer(nn.Module):
    def __init__(self, 
        ...
        self.linear_mapper = nn.Linear(self.input_d, self.hidden_dim, bias=False)
        ...
        self.patchify = ImagePreprocesser(n_patches=n_patches)
